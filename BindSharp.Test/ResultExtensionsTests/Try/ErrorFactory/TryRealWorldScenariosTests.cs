...
public sealed class TryRealWorldScenariosTests
{
    ...
    {
        // Act
        var successResult = ResultExtensions.Try(
            () => int.Parse("123"),
            ex => $"Failed to parse number: {ex.Message}");

        var failureResult = ResultExtensions.Try(
            () => int.Parse("abc"),
            ex => $"Failed to parse number: {ex.Message}");
    ...
    public async Task TryAsync_WithErrorFactory_HttpRequestScenario()
    {
        ...
        // Act
        var successResult = await ResultExtensions.TryAsync(
            () => FetchDataAsync(true),
            ex => $"API call failed: {ex.Message}");

        var failureResult = await ResultExtensions.TryAsync(
