...
public sealed class TryAsyncTests
{
    ...
    public async Task TryAsync_WithErrorFactory_WhenOperationThrows_ReturnsFailureWithTransformedError()
    {
        ...
        // Act
        var result = await ResultExtensions.TryAsync<bool, string>(
            async () =>
            {
                await Task.Delay(1);
                throw exception;
    ...
    {
        // Act
        var result1 = await ResultExtensions.TryAsync<bool, string>(
            async () =>
            {
                await Task.Delay(1);
        ...
        var result2 = await ResultExtensions.TryAsync<bool, string>(
            async () =>
            {
                await Task.Delay(1);
