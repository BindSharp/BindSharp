namespace BindSharp.Test.ResultExtensionsTests.Try;
...
public sealed class CombinedTryTests
{
    ...
    public void Try_BothOverloads_CanCoexist()
    {
        ...
        var result1 = ResultExtensions.Try(() => 42);
        
        // The error-factory overload
        var result2 = ResultExtensions.Try(
            () => 42,
            ex => $"Error: {ex.Message}");
    ...
    {
        // The exception-first overload
        var result1 = await ResultExtensions.TryAsync(async () =>
        {
            await Task.Delay(1);
            return "test";
        });
        ...
        // The error-factory overload
        var result2 = await ResultExtensions.TryAsync(
            async () =>
            {
                await Task.Delay(1);
                return "test";
